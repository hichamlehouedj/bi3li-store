...
import { Card, Text, Group, Button, ActionIcon, useMantineTheme, Grid, TextInput, Select, NumberInput, NumberInputHandlers, Accordion, Stack, Box, Radio, Notification, rem, Alert, CheckIcon, Checkbox, Chip } from '@mantine/core';
...
export function FormOrder({data}: Props) {
    ...
    const {onSubmit, reset, values, getInputProps, errors, setValues} = useForm({
        initialValues: {
            name: '', phone: "", state: "", city: "", quantity: 1, typeFee: "desk_fee", address: "", sizes: [], colors: []
        },
        validate: {
    ...
    useEffect(() => {
        ...
        if (values.quantity < values.sizes.length) {
            setValues({sizes: []})
        }
        if (values.quantity < values.colors.length) {
            setValues({colors: []})
        }
    }, [values.quantity])
    ...
    const onSubmitForm = ({name, phone, state, city, typeFee, address, sizes, colors}: any) => {
        const {_id} = data
        ...
        .then(({data}) => {
            ...
            // for (let i = 0; i < facebookApis.length; i++) {
            //     const facebookApi = facebookApis[i];
            //     if (facebookApi?.apiKey !== "" && facebookApi?.apiKey.length > 10) {
            //         ReactPixel.trackSingle(facebookApi?.apiKey, 'Purchase', {
            //             order_id: data?._id
            //         })
            //     }
            // }

            navigate(`/order/confirmed/product/${_id}`)
            // setTealAlert(true)
            // setRedAlert(false)
            onReset()
        })
        .catch((error) => {
            console.log(error)
            navigate(`/order/failed/product/${_id}`)
            // setTealAlert(false)
            // setRedAlert(true)
        });
    }
    ...
    return (
        <Card withBorder radius="md" p={30} style={{border: `2px solid ${dataStore?.information?.backgroundColor || "#645cbb"}`}}>
                                    ...
                                    {data?.colors.map((item: any, index: number) => (
                                        <Checkbox
                                            ...
                                            icon={({ indeterminate, ...others }) =>
                                                indeterminate ? <IconMinus {...others} /> : <IconCheck stroke={2} color={["#FFFFFFFF", "#9E9E9EFF"].includes(item) ?  "#000" : "#fff"} {...others} />
                                            }
                                        />
                                    ))}
                                </Group>
